...
export class SessionsStore {
  ...
  get activeSession(): SessionDescriptor | null {
    ...
    return (
      this.sessions.find((s) => s.sessionId === this.activeSessionId) ?? null
    );
  }
  ...
  async createSession(
    ...
    try {
      const response = await invoke<RpcResponse<CreateSessionData>>(
        "create_agent",
        {
          projectDir,
          provider,
      ...
      const resolvedSessionFile =
        ...
        response.data.sessionFile.trim().length > 0
  ...
  async closeSession(sessionId: string): Promise<void> {
    ...
    if (this.activeSessionId === sessionId) {
      this.activeSessionId =
        nextSessions.length > 0 ? (nextSessions[0]?.sessionId ?? null) : null;
    }
  }
  ...
  async refreshFromBackend(): Promise<void> {
    ...
    this.sessions = sessions
      .map((session): SessionDescriptor | null => {
        ...
        const modelProvider =
          session.model && typeof session.model.provider === "string"
            ? session.model.provider
            : null;
        const modelId =
          session.model && typeof session.model.id === "string"
            ? session.model.id
            : null;
        const sessionFile =
    ...
    if (
      this.activeSessionId &&
      this.sessions.some((s) => s.sessionId === this.activeSessionId)
    ) {
      return;
    }
