...
<div class={cn(
  ...
  <div class={cn(
    "w-full bg-card border border-border rounded-lg px-5 py-4 wrap-break-word",
    ...
    {#each content as block (block.type === 'toolCall' ? block.id : block)}
      {#if isThinkingBlock(block)}
        {@const blockIndex = content.indexOf(block)}
        <div class="mb-3 bg-foreground/3 dark:bg-f6fff5/[0.03] border border-border rounded overflow-hidden">
          <button 
            type="button"
            class="flex items-center justify-between w-full gap-2 px-3 py-2 bg-foreground/5 dark:bg-f6fff5/[0.05] border-b border-border text-xs font-semibold text-muted-foreground uppercase tracking-wider hover:bg-foreground/8 dark:hover:bg-f6fff5/[0.08] transition-colors cursor-pointer"
          ...
          >
            ...
            <svg 
              ...
              class:rotate-90={!thinkingCollapsed[blockIndex]}
              viewBox="0 0 24 24" 
              fill="none" 
              stroke="currentColor"
            >
          ...
          {#if !thinkingCollapsed[blockIndex]}
      ...
      {:else if isTextBlock(block)}
        <div class="text-[0.9375rem] leading-relaxed text-foreground whitespace-pre-wrap wrap-break-word mt-3 first:mt-0">{block.text}</div>
      {/if}
    {/each}
  </div>
</div>
